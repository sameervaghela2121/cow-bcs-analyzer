...
describe('milkingGcsService', () => {
  ...
  it('builds the organizationId/facilityId/dateFolder/filename object path', () => {
    const objectPath = buildMilkingObjectPath({
      organizationId: 'org1',
      facilityId: 'fac1',
      dateFolder: '2026-07-22',
      filename: 'scr-2026-07-22.xlsx',
    });
    expect(objectPath).toBe('org1/fac1/2026-07-22/scr-2026-07-22.xlsx');
  });

  it('rejects an unsafe dateFolder or filename', () => {
    expect(() =>
      buildMilkingObjectPath({ organizationId: 'org1', facilityId: 'fac1', dateFolder: '../etc', filename: 'a.xlsx' })
    ).toThrow();
    expect(() =>
      buildMilkingObjectPath({ organizationId: 'org1', facilityId: 'fac1', dateFolder: '2026-07-22', filename: '../../a.xlsx' })
    ).toThrow();
  });

  it('converts an object path to a gs:// URI in the milking bucket', () => {
    const objectPath = 'org1/fac1/2026-07-22/scr.xlsx';
  ...
  it('generates a v4 signed PUT URL against the milking bucket, bound to the given content type', async () => {
    ...
    const url = await generateMilkingUploadUrl({ objectPath: 'org1/fac1/2026-07-22/scr.xlsx', contentType });
